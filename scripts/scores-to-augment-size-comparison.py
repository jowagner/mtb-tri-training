...
while True:
    ...
    augsize_types.add(augsize)
    graph_key = (language, parser, augsize)
    setting_key = (oversampling, sample, agreement, decay)
...
for lang_index, language in enumerate(sorted(list(languages))):
    ...
    for parser in target_parsers:
        for augsize in target_augsizes:
            graph_key = (language, parser, augsize)
            if not graph_key in graphs:
        ...
        for augsize in sorted(list(augsize_types)):
            ...
            for setting_key in sorted(list(eligible_setting_keys)):
                ...
                for run_index, rounds in enumerate(min_rounds):
                    if augsize not in target_augsizes:
                        setting.sort()
                    if len(setting) > run_index:
                        neg_n_rounds, run, exp_code, row = setting[run_index]
            ...
            for setting_key in sorted(list(eligible_setting_keys)):
                ...
                for run_index, rounds in enumerate(min_rounds):
                    ...
                    if len(setting) > run_index:
                        ...
                        sys.stderr.write('\t__%s%s%s%s__\t%s\t%d\t%.9f\t(best of %d scores)\n' %(tuple(setting_key) + (augsize, run_index, best_score, scores_tested)))
            if best_scores:
                average_score = sum(best_scores) / float(len(best_scores))
                sys.stderr.write('\taverage best score for %s: %.9f (average of %d scores)\n' %(augsize, average_score, len(best_scores)))
            if augsize in target_augsizes:
...
for augsize in sorted(list(set(target_augsizes) & augsize_types)):
    header.append('\\textbf{%s}' %augsize)
if len(target_augsizes) == 2:
