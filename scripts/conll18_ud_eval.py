...
#
# Minor modifications: Joachim Wagner, Dublin City University
# see
# https://github.com/tira-io/ADAPT-DCU/blob/tbemb/scripts/conll18_ud_eval.py
# for a history of my changes
#
# Changelog:
...
def load_conllu(file):
    ...
    class UDRepresentation:
        def __init__(self):
            ...
            # TODO: What about MWT and other special tokens?
            self.characters = []
...
def evaluate(gold_ud, system_ud, include_sentence_level_info = False):
    ...
    def get_words_for_sentence(ud_words, ud_sentence):
        ...
        for word in ud_words:
            if word.span.start < ud_sentence.end \
            and word.span.end > ud_sentence.start:
                retval.append(word)
        return retval
    if not include_sentence_level_info:
        # force below loop to not process any items
        num_sent_gold = 0
    sentence_scores = []
    for sent_i in range(min(num_sent_gold, num_sent_system)):
        gold_ud_words = get_words_for_sentence(
            gold_ud.words, gold_ud.sentences[sent_i]
        )
        system_ud_words = get_words_for_sentence(
            system_ud.words, system_ud.sentences[sent_i]
        )
        alignment = align_words(gold_ud_words, system_ud_words)
        sent_score = alignment_score(
            alignment,
        ...
        for metric in["Tokens", "Sentences", "Words", "UPOS", "XPOS", "UFeats", "AllTags", "Lemmas", "UAS", "LAS", "CLAS", "MLAS", "BLEX"]:
            if args.counts:
                ...
                print("{:11}|{:14.9f} |{:14.9f} |{:14.9f} |{}".format(
                    ...
                    "{:14.9f}".format(100 * evaluation[metric].aligned_accuracy) if evaluation[metric].aligned_accuracy is not None else ""
                ))
    if args.sentences:
        print("Sentence\tLAS\tlen(gold)\tlen(predicted)")
        for sentence_score in evaluation['PerSentenceLAS']:
            print('%d\t%.9f\t%d\t%d' %sentence_score)
